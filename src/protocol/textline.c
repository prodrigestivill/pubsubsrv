 ...
 * 
 * textline.c is free software: you can redistribute it and/or modify it
 * under the terms of the GNU General Public License as published by the
 * Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 * 
 * textline.c is distributed in the hope that it will be useful, but
...
void protocol_textline_input(struct client *from, char buf[], int len){
	...
	if (l<1 || (l==1 && buf[0]=='*')){
		...
		if (t!=0)
			...
			r = write(from->connection, "ERR: Topic not set for publishing.\r\n", 36);
	}
}
